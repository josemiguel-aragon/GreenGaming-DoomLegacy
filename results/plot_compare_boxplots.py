...
input_files = ["-O0", "-O1", "-O2", "-O3", "-Os", "-Oz", "ga_doom"]
bench = "doomlegacy"

if __name__ == '__main__':
    labels = ["-O0", "-O1", "-O2", "-O3", "-Os", "-Oz", "ga_doom"]
    tags = ["-O0", "-O1", "-O2", "-O3", "-Os", "-Oz", "cGA"]
    ...
    for i,x in enumerate(input_files):
        ...
        data_time.append(np.loadtxt(f'solutions_ec_fitness_{x}_{bench}.csv', delimiter=',') / 1000)
        ...
        if x in "ga_doom":
            print((np.median(data_time[-1])- np.median(data_time[3]))/np.median(data_time[3]) * 100)
    ...
    df2.to_csv("ec_measurements_df.csv")
    ...
    plt.savefig("./improvementbpDoomLegacy-ec.png", dpi=300)    