...
class IntegerSPXCrossover(Crossover[IntegerSolution, IntegerSolution]):

    def __init__(self, probability: float):
        super(IntegerSPXCrossover, self).__init__(probability=probability)
    ...
    def execute(self, parents: List[IntegerSolution]) -> List[IntegerSolution]:
        ...
        if rand <= self.probability:
            ...
            total_number_of_variables = parents[0].number_of_variables

            # 2. Calculate the point to make the crossover
            variable_to_cut = random.randrange(0, total_number_of_variables)

            # 3. Apply the crossover to the variable
            for i in range(variable_to_cut + 1, total_number_of_variables):
                swap = offspring[0].variables[i]
                offspring[0].variables[i] = offspring[1].variables[i]
                offspring[1].variables[i] = swap
    ...
    def get_name(self) -> str:
        return 'Integer Single point crossover'


class IntegerTPXCrossover(Crossover[IntegerSolution, IntegerSolution]):

    def __init__(self, probability: float):
        super(IntegerTPXCrossover, self).__init__(probability=probability)
